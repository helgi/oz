...
class Guest(object):
    ...
    def __init__(self, tdl, config, auto, output_disk, nicmodel, clockoffset,
        ...
        if os.uname()[4] in ["i386", "i586", "i686"] and self.tdl.arch == "x86_64":
            raise OzException("Host machine is i386, but trying to install x86_64 guest; this cannot work")
        ...
        self.screenshot_dir = config_get_key(config, 'paths', 'screenshot_dir', oz.ozutil.default_screenshot_dir(self.data_dir))
        self.sshprivkey = config_get_key(config, 'paths', 'sshprivkey', oz.ozutil.default_sshprivkey())
        ...
        self.jeos_filename = join(self.jeos_cache_dir, self.tdl.distro + self.tdl.update + self.tdl.arch  + '.' + jeos_extension)
    ...
    def _internal_generate_diskimage(self, size=10, force=False,
        ...
        directory = dirname(diskimage)
        filename = basename(diskimage)
